...
Min_char = 0.01
Max_char = 0.09
image_path = 'data/test/images/29AA99999.jpg'
...
#    print(f"Lớp: {name} | Kiểu dữ liệu: {param.dtype}")

source_img = cv2.imread(image_path)
pred, LP_detected_img = detect(model_LP, source_img, device, imgsz=640)

cv2.imwrite('input.jpg', cv2.resize(source_img, dsize=None, fx=0.5, fy=0.5))
cv2.imwrite('LP_detected_img.jpg', cv2.resize(LP_detected_img, dsize=None, fx=0.5, fy=0.5))
...
for *xyxy, conf, cls in reversed(pred):
    ...
    if (rotate_thresh is None) or (LP_rotated is None):
        continue
    cv2.imwrite('LP_rotated.jpg', LP_rotated)
    cv2.imwrite('rotate_thresh.jpg', rotate_thresh)
    ...
    cv2.imwrite('LP_rotated_copy.jpg', LP_rotated_copy)
    ...
    cv2.imwrite('charac.jpg', LP_rotated_copy)
    cv2.imwrite('LP_rotated_{}.jpg'.format(c), LP_rotated)
    print('License Plate_{}:'.format(c), strFinalString)
    c += 1

cv2.imwrite('final_result.jpg', cv2.resize(LP_detected_img, dsize=None, fx=0.5, fy=0.5))
print('Finally Done!')
cv2.waitKey(0)
